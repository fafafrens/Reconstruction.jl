# Included inside Reconstruction after the slope-limiter definitions.
...
and CWENO5 (five samples). `dx` is the physical cell width.
"""
...
@inline _pad_quadratic(a) = (a[1], a[2], a[3], zero(a[1]), zero(a[1]))
...
WENO3, WENOZ, and MP5 expose face reconstructions only.
"""
...
@inline function cell_polynomial(recon::_PolynomialReconstruction, stencil::AbstractVector{<:Number})
    ...
    vals = ntuple(j -> stencil[firstindex(stencil) + j - 1], Val(n))
